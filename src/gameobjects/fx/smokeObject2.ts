...
export class SmokeObject2 extends ParticleEmitter {
    ...
    private emitParticles(emitPosition: THREE.Vector3): void {
        for(let i = 0; i < this.numberParticles; i++) {
            ...
            sprite.userData.velocity = new THREE.Vector3(
                Math.random() * 0.01 - 0.005, //this.velocity - this.velocity / 2,
                Math.random() * this.velocity,
                Math.random() * 0.01 - 0.005, //this.velocity - this.velocity / 2
            );
            sprite.userData.velocity.multiplyScalar(Math.random() * Math.random() * 3 + 2);

            sprite.material.color =  new THREE.Color('gray'); //this.particleColor;
    ...
    update(): void {
        ...
        this.particleGroup.children.forEach((child) => {
            ...
            if(item.material.opacity < 0.98 && item.material.opacity >= 0.80)      
                item.material.color.lerp(new THREE.Color('gray'), 0.5);
            else if(item.material.opacity < 0.80 && item.material.opacity >= 0.50)      
                item.material.color.lerp(new THREE.Color(0x663399), 0.5);
